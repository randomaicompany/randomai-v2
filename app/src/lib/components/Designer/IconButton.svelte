<script>
  ...
  $: restingLabelClasses = (() => {
    if (isbackground) {
      const anotherButtonIsActive = currentActiveButtonId !== null && currentActiveButtonId !== id;
...
<button
  ...
  {#if label}
    <p
      class="absolute select-none pointer-events-none whitespace-nowrap text-xs border border-brand-accent bg-brand-accent text-white p-1 pb-[5px] leading-none px-2 rounded-xl z-[1] transition-all duration-400 left-[50%] top-[-45px] -translate-x-1/2 translate-y-3
