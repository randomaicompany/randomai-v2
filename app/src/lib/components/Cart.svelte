<script>
	...
	/** @type {Props} */
	let { isLoading = $bindable(false), isCartOpen = $bindable(false) } = $props();
...
{#if isCartOpen}
	...
	>
		...
		>
			...
			<button class="p-1" onclick={close}>
				...
				>
					...
					<div class="flex gap-2">
						{#if frontImageUrl || backImageUrl}
							...
							>
								{#if frontImageUrl}
									<img alt={lineItem.title} class="h-16 w-16 object-contain" src={frontImageUrl} />
								{:else}
							...
							>
								{#if backImageUrl}
									<img alt={lineItem.title} class="h-16 w-16 object-contain" src={backImageUrl} />
								{:else}
					...
					<div class="w-full">
						...
						<div class="flex items-center justify-between">
							...
							<ul class="mt-4 inline-flex items-center">
								<li>
									<button
										onclick={decrement(lineItem.variant.id)}
										disabled={isLoading}
