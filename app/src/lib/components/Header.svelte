...
<header class:top-9={hasNoticeText} class="sticky top-0 z-10 bg-white border">
  <nav class="shell flex items-center w-full h-20 gap-4">
    <button
      on:click={toggleMenu}
      class="relative xl:hidden flex items-center justify-center"
    >
      {#if isMenuOpen}
        <i class="material-symbols-rounded">close</i>
        <div
          class="absolute w-9 h-9 bg-brand-primary/10 -top-1.5 -left-1.5 rounded-full"
        ></div>
      {:else}
    ...
    <a href="/">
      <PrismicImage
        class="object-contain w-full h-5 min-w-24"
