...
{#if hasNoticeText}
  <div
    class="sticky top-0 z-10 flex h-9 w-full items-center justify-center bg-brand-accent text-sm text-white">
  ...
  <nav class="shell flex h-20 w-full items-center gap-4">
    <button
      ...
      {#if isMenuOpen}
        ...
        <div
          class="absolute -left-1.5 -top-1.5 h-9 w-9 rounded-full bg-brand-primary/10">
        </div>
      {:else}
    ...
    <a href="/">
      <PrismicImage
        class="h-5 w-full min-w-24 object-contain"
    ...
    <form onsubmit={preventDefault(bubble('submit'))} class="hidden md:block">
      <label class="input-field-container">
        ...
        <button type="submit">
          <i
            class="material-symbols-rounded absolute left-4 top-2.5 h-6 opacity-55">
            search
          </i>
        </button>
      </label>
      ...
      <li class="flex h-[42px] w-[42px] items-center justify-center">
        <button
          ...
          {#if $cart?.lineItems?.length}
            <p
              class="absolute -top-1 right-2 flex h-4 min-w-[1rem] items-center justify-center rounded-full bg-brand-accent-light text-xs leading-none text-white">
