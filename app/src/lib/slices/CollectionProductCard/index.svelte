<script>
	 ...
	 * @property {import("@prismicio/client").Content.CollectionProductCardSlice} slice
	 */
...
<div class="relative" data-slice-type={slice.slice_type} data-slice-variation={slice.variation}>
	...
	>
		{#each slice.items as { image }, index (index)}
			<SplideSlide>
				<div class="zoom-container relative overflow-hidden" bind:this={zoomContainers[index]}>
					{#if slice.primary.link_text?.[0]?.text.trim().toLowerCase() !== 'buy now'}
						<a href={editorLink}>
							<PrismicImage
								class="bg-brand-smoke-darker h-96 w-full object-cover object-top"
								field={image}
	...
	<ul class="flex flex-wrap gap-2 py-4">
		{#each slice.items as { swatch }, idx (idx)}
	...
	<div class="flex min-h-40 flex-col">
		<div class="flex-1">
			...
			<p class="text-brand-gray text-brand-primary/70 mt-2 font-normal">
