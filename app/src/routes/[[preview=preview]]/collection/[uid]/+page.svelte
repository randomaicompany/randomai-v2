...
<div
  class:!gap-0={!isFilterPanelOpen}
  class="shell flex items-start gap-12 overflow-x-hidden">
  <aside
    ...
    <div class="relative rounded-md border border-gray-200 p-5">
      <div>
        <div class="-mt-1.5 mb-4 inline-flex items-center">
          ...
          <button
            ...
            ><i class="material-symbols-rounded rounded-full !text-xl">close</i
            ></button>
        </div>
      ...
      <div class="pt-8">
        <CheckboxGroup
          label="Gender"
          options={page.genders}
          bind:values={filters["genders"]} />
      </div>
  ...
  <div class="pt-1">
    <button
      ...
      class="mb-4 ml-auto flex items-center justify-between gap-1 rounded-md bg-brand-accent-light px-4 py-1.5 pl-2 text-white">
      <i
        class:rotate-180={!isFilterPanelOpen}
        class="material-symbols-rounded !text-xl">menu_open</i>

      <p class="text-sm">Filters</p>
    </button>

    <div
      class="grid gap-4 gap-y-16 pb-32 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
      {#each productCardSlices as slice}
        ...
        {@const isGenderMatch = R.includes(
          slice.primary.gender,
          filters.genders
        )}
        {@const isCollectionAMatch = R.includes(
          slice.primary.collection,
          filters.collections
        )}
        {@const isProductTypeAMatch = R.includes(
          slice.primary.product_type,
          filters.productTypes
        )}
        {@const isProductColorAMatch = R.isEmpty(
          R.difference(getColors(slice), filters.colors)
        )}
