<script>
  ...
  $: productResults =
    data?.results?.filter((doc) => doc.type === "product") || [];
  ...
  <ul class="grid grid-cols-2 gap-4 pt-8 md:grid-cols-3 lg:grid-cols-4">
    {#each productResults as page}
      {#if page?.uid}
        <li>
          <PrismicLink
            class="block rounded-md border border-gray-300 bg-white p-4"
            field={page}>
            ...
            <p
              class="mb-4 flex max-w-[max-content] justify-start rounded-sm bg-brand-smoke-darker p-2 py-1 text-xs uppercase">
              {page.type}
            </p>
            <!-- <div class="mb-8">
