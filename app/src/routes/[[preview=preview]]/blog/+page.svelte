<script>
  ...
  const heroSlice = R.filter(
    R.propEq("hero", "slice_type"),
    data.page.data.slices
  );

  const openModal = (item) => () => (
    (previewItem = item), (isModalOpen = true)
  );
...
<div class="relative mx-auto mb-8 max-w-7xl">
  ...
  <button
    on:click={scrollLeft}
    disabled={isAtStart}
    class="top absolute -top-[40px] right-10 z-10 cursor-pointer rounded-full text-brand-accent-light"
  ...
  <button
    ...
    disabled={isAtEnd}
    class="absolute -top-[40px] right-3 cursor-pointer rounded-full bg-white text-brand-accent-light"
  ...
  <ul
    ...
    class="flex items-start gap-4 overflow-x-auto scroll-smooth rounded bg-white p-2 scrollbar-hide">
    {#each data.feeds as feed}
      <li>
        <a
          href={`#${feed.name}`}
          class="block whitespace-nowrap rounded border bg-gray-50 px-4 py-2 text-sm font-medium hover:text-brand-accent">
          {feed.name}
        </a>
      </li>
    {/each}
        ...
        {#if R.not(R.isEmpty(feed.items))}
          <Splide {options}>
            {#each feed.items as item}
              <SplideSlide>
                <div
                  class=" flex h-[42.5rem] flex-col border border-gray-300 bg-white p-4">
                  <div class="flex-1">
                    ...
                    <div class="relative">
                      ...
                      <ul class="mt-4 flex flex-wrap items-center gap-2">
                        {#each R.slice(0, 3, item.categories) as category}
                          <li>
                            <p
                              class="inline-block rounded bg-brand-tertiary/30 px-3 py-1 text-xs">
                              {category}
                            </p>
                          </li>
                        {/each}
                  ...
                  <button
                    on:click={openModal({ ...feed, ...item })}
                    class="button-compact mt-4">Read More</button>
                </div>
              </SplideSlide>
            {/each}
          </Splide>
...
{#if isModalOpen}
  <div
    ...
    class="fixed left-0 top-0 z-10 flex h-full w-full items-start justify-center overflow-auto bg-brand-black/50 p-4">
    <div
      ...
      <button
        on:click={closeModal}
        class="absolute -top-8 right-0 bg-white p-1 px-2"
      ...
      <div class="relative">
        ...
        <a target="_blank" href={previewItem.link} class="button-compact mt-16"
          >View Full Story</a>
      </div>
    </div>
  </div>
